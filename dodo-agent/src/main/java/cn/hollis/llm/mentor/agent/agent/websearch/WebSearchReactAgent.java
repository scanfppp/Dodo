...
public class WebSearchReactAgent extends BaseAgent {
    ...
    private final List<Advisor> advisors; // 可用顾问列表
    private final int maxReflectionRounds; // 最大反思轮数

    private static final ObjectMapper MAPPER = new ObjectMapper(); // JSON 映射器
    ...
    public WebSearchReactAgent(String name, ChatModel chatModel, List<ToolCallback> tools, String systemPrompt, int maxRounds,
        ...
        this.sessionService = sessionService; // 会话服务
        this.taskManager = taskManager; // 任务管理器
    ...
    private void initChatClient() {
        try {
            ...
            ToolCallingChatOptions toolOptions = ToolCallingChatOptions.builder()
                    .toolCallbacks(tools) // 注册工具，注册可用的工具（如 Tavily 搜索）
                    .internalToolExecutionEnabled(false) // 禁用内部工具执行，禁用内部工具执行，由自己处理工具调用逻辑
                    .build();

            ChatClient.Builder builder = ChatClient.builder(chatModel);
            if (!CollectionUtils.isEmpty(advisors)) {
                builder.defaultAdvisors(advisors); // 注册顾问，添加顾问增强（如日志、重试等）
            }
     ...
     * 流式输出
     */
    public Flux<String> stream(String question) { // 无会话记忆的流式输出
    ...
    private Flux<String> streamInternal(String conversationId, String question) {
        ...
        // 初始化流式输出
        // Sink 是Reactor 提供的 手动发布数据 的工具。many发送多个元素，unicast单播 模式，这个 Sink 只能被 一个订阅者订阅 。onBackpressureBuffer() 是一个背压缓冲区，用于处理订阅者处理能力不足的情况
        // 该缓冲区会缓存数据，直到订阅者处理能力恢复到正常水平。
    ...
    private void scheduleRound(List<Message> messages, Sinks.Many<String> sink, AtomicLong roundCounter, AtomicBoolean hasSentFinalResult,
        ...
        Disposable disposable = chatClient.prompt()
                ...
                .stream()
                .chatResponse()
                //切换执行线程到 boundedElastic 调度器。- AI 模型调用是阻塞 I/O 操作需要在专门的线程池执行，避免阻塞主线程boundedElastic 是 Spring AI 推荐的调度器
